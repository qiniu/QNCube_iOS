...
- (void)creatSubViewTime
{
    ...
    _joinLabel.hidden        = YES;
    [self.contentView addSubview:_joinLabel];
    _joinLabel.font=[UIFont fontWithName:FONT_REGULAR size:10];
    _joinLabel.backgroundColor=COLOR_999999;
    _joinLabel.textColor=[UIColor lightTextColor];
    _joinLabel.textAlignment=NSTextAlignmentCenter;
    _joinLabel.layer.masksToBounds=YES;
    _joinLabel.layer.cornerRadius=4;
    _joinLabel.layer.borderColor=[COLOR_999999 CGColor];
    _joinLabel.layer.borderWidth=1;
}
...
- (void)setMessageModel:(CSMessageModel *)messageModel {
    ...
    _joinLabel.hidden = !messageModel.showMessageTime;
    _joinLabel.frame = [messageModel timeFrame];
    _joinLabel.text = messageModel.messageTime;
    
    _logoImageView.hidden = messageModel.showMessageTime;
    _logoImageView.frame = [messageModel logoFrame];
   
    _bubbleImageView.hidden = messageModel.showMessageTime;
    _bubbleImageView.frame = [messageModel bubbleFrame];
    
    if (messageModel.messageSenderType == MessageSenderTypeMe)
    {
        _logoImageView.image = [UIImage imageNamed:@"icon_image"];
