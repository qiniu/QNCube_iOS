...
- (CGRect)messageFrame
{
    ...
    if (self.messageSenderType == MessageSenderTypeMe)
    {
        rect = CGRectMake(ScreenWidth - size.width - 70, timeRect.size.height + 10, size.width, size.height > 44 ? size.height : 44);;
    }
    else
    {
        rect = CGRectMake(65 , timeRect.size.height + 10 , size.width, size.height > 44 ? size.height : 44);
    }
    return rect;
}
